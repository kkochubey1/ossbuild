...
public class PackageTest {
	...
	public void testRegistry() {
		...
		assertTrue(Sys.loadNativeResources(NativeResource.OpenGL));
		...
		switch(Sys.getOSFamily()) {
			case Unix:
				assertTrue(Path.exists(Path.combine(binDir, "libGLEW.so.1.5")));
				break;
			case Windows:
				assertTrue(Path.exists(Path.combine(binDir, "glew32.dll")));
				break;
			default:
