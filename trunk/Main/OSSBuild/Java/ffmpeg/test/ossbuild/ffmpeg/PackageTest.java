...
public class PackageTest {
	...
	public void testRegistry() {
		...
		assertTrue(Sys.loadNativeResources(NativeResource.FFmpeg));
		...
		switch(Sys.getOSFamily()) {
			case Unix:
				assertTrue(Path.exists(Path.combine(binDir, "libavfilter-gpl.so.1")));
				break;
			case Windows:
				assertTrue(Path.exists(Path.combine(binDir, "avfilter-gpl-1.dll")));
				break;
			default:
