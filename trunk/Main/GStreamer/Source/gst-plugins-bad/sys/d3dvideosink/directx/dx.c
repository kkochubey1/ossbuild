...
directx_initialize(DirectXInitParams* init_params) 
{
  ...
  dx.supported = (dx.best_api != NULL && !DIRECTX_VERSION_IS_UNKNOWN(dx.best_api->version));
  dx.initialized = TRUE;
...
void 
directx_log_debug(const gchar* file, const gchar* function, gint line, const gchar * format, ...) 
{
...
void 
directx_log_warning(const gchar* file, const gchar* function, gint line, const gchar * format, ...) 
{
...
void 
directx_log_error(const gchar* file, const gchar* function, gint line, const gchar * format, ...) 
{
...
directx_determine_best_available_api (void)
{
...

  {
    ...
    DIRECTX_LOCK
    {
      ...
      DIRECTX_DEBUG("Searching supported DirectX APIs for the best (most recent) one available");
      for(item = g_list_first(dx.supported_api_list); item; item = item->next) {
        ...
        DIRECTX_DEBUG("Searching for module \"%s\" with the symbol \"%s\"", dxlib->module_test, dxlib->symbol_test);

        /* Can we locate and open a Direct3D library (e.g. d3d9.dll or d3d10.dll)? */
        if ((lib = g_module_open(dxlib->module_test, G_MODULE_BIND_LAZY)) != NULL) {
