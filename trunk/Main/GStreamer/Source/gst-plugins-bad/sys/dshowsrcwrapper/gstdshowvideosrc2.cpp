...
gst_dshowvideosrc_set_caps (GstBaseSrc * bsrc, GstCaps * caps)
{
  ...
  GList *type_pin_mediatype;
  GstCapturePinMediaType *pin_mediatype;
  VIDEOINFOHEADER *video_info = NULL;
  guint i = 0;
  gint mediatype_index = -1;
  gint width, height;
  gint fps_n, fps_d;

  if (!gst_caps_can_intersect (src->caps, caps))
    goto incompatible_caps;

  caps = gst_caps_intersect (src->caps, caps);
  
  for (; i < gst_caps_get_size (src->caps) && mediatype_index == -1; i++) {
    GstCaps *capstmp = gst_caps_copy_nth (src->caps, i);
    if (gst_caps_is_subset (caps, capstmp)) {
      mediatype_index = i;
    }
  ...
  type_pin_mediatype = g_list_nth (src->pins_mediatypes, mediatype_index);

  pin_mediatype = (GstCapturePinMediaType *) type_pin_mediatype->data;
  ...
  video_info->AvgTimePerFrame =
      (LONGLONG) (10000000 * fps_d / (double) fps_n);
  ...
  if (hres != S_OK)
    goto cannot_connect_filter;

  /* save width and height negociated */
  src->width = width;
  src->height = height;
  src->is_rgb = gst_video_format_is_rgb (format);
...
incompatible_caps:
  {
    GST_ERROR_OBJECT (src, "caps incompatible");
    return FALSE;
  }
incomplete_caps:
  {
    GST_DEBUG_OBJECT (src, "Failed to retrieve either width, "
...
invalid_format:
  {
    GST_DEBUG_OBJECT (src,
        "Could not get format from caps %" GST_PTR_FORMAT, caps);
    return FALSE;
  }
no_input_pin:
  {
