...
public class PackageTest {
	...
	public void testRegistry() {
		...
		assertTrue(Sys.loadNativeResources(NativeResource.Fonts));
		...
		switch(Sys.getOSFamily()) {
			case Unix:
				assertTrue(Path.exists(Path.combine(binDir, "libfontconfig.so.1")));
				break;
			case Windows:
				assertTrue(Path.exists(Path.combine(binDir, "libfontconfig-1.dll")));
				break;
			default:
