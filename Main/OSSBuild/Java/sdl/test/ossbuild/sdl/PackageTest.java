...
public class PackageTest {
	...
	public void testRegistry() {
		...
		assertTrue(Sys.loadNativeResources(NativeResource.SDL));
		...
		switch(Sys.getOSFamily()) {
			case Unix:
				assertTrue(Path.exists(Path.combine(binDir, "libSDL-1.2.so.0")));
				break;
			case Windows:
				assertTrue(Path.exists(Path.combine(binDir, "SDL.dll")));
				break;
			default:
