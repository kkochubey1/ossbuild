...
main (int argc, char *argv[])
{
  ...
  app->audio_sink = gst_parse_bin_from_description(audio_sink_desc, TRUE, &err);
  if (!app->audio_sink) {
    GST_PLAYER_GST_ERROR_MSG("Initialization error", err);
    exit(ERROR_INITIALIZATION);
  }

  app->video_sink = gst_parse_bin_from_description(video_sink_desc, TRUE, &err);
  if (!app->video_sink) {
    GST_PLAYER_GST_ERROR_MSG("Initialization error", err);
    exit(ERROR_INITIALIZATION);
  }
  ...
  if (app->ping_interval > 0 && app->command_mode) {
    ...
    while(!ping_is_started)
      g_cond_wait(ping_started, ping_lock);
    GST_PLAYER_PING_UNLOCK
