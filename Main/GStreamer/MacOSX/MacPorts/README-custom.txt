...
2) Cleaning-up step
The folder with the required dylib and so files is <prefix>/lib. Inside lib you can delete all subfolders except gstreamer-0.10, which contains the plugins. All the files that are not either dylib and so can be deleted.

3) Post-processing step

3.1) Replace absolute paths with relative paths using the replacepath.py tool.
For base libs:
...
NOTE: Don't leave the original library files in the original location!!!!!!!!!!!

3.2) If also needed, eliminate symbolic links using the removesymlinks.py tool:
